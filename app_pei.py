        ...
        with tabs[2]:
            ...
            if st.button("👁️ GERAR PDI COMPLETO (PDF)", use_container_width=True):
                ...
                for i, item in enumerate(checklist_items_fam):
                    ...
                    h = max(6, calcular_linhas(pdf, obs, 38) * 4) + 2
                    ...
                    pdf.set_xy(x+141, y+1); pdf.multi_cell(38, 4, clean_pdf_text(obs), 0, 'L')
                    pdf.set_y(y+h)
                ...
                def pdf_table_row(pdf_obj, domain, p_text, f_text, w1=60, w2=60, w3=60):
                    pdf_obj.set_font("Arial", "", 8)
                    l1 = calcular_linhas(pdf_obj, domain, w1 - 2)
                    l2 = calcular_linhas(pdf_obj, p_text, w2 - 2)
                    l3 = calcular_linhas(pdf_obj, f_text, w3 - 2)
                    h = max(l1, l2, l3) * 5 + 4
                    ...
                    if pdf_obj.get_y() + h > 270:
                        ...
                        pdf_obj.cell(w2, 8, "Resultados (1º Semestre)", 1, 0, 'C', True)
                        pdf_obj.cell(w3, 8, "Resultados (2º Semestre)", 1, 1, 'C', True)
                        pdf_obj.set_font("Arial", "", 8)
                    ...
                    pdf_obj.multi_cell(w1-2, 5, clean_pdf_text(domain), 0, 'L')
                    pdf_obj.set_xy(x+w1+1, y+2)
                    pdf_obj.multi_cell(w2-2, 5, clean_pdf_text(p_text), 0, 'L')
                    pdf_obj.set_xy(x+w1+w2+1, y+2)
                    pdf_obj.multi_cell(w3-2, 5, clean_pdf_text(f_text), 0, 'L')
