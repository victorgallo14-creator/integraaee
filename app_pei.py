...
with st.sidebar:
    ...
    <style>
        ...
        [data-testid="stSidebar"] [data-testid="stVerticalBlock"] {
            gap: 1.2rem !important; /* Espaço mínimo entre itens */
        }
...
# --- 5. AVALIAÇÃO ACADÊMICA (PDF ADAPTADO) ---
            ...
            def print_meta_row(titulo, meta, estrategia):
                ...
                pdf.set_font("Arial", "B", 10); pdf.cell(0, 5, clean_pdf_text("Estratégias:"), "LTR", 1)
...
Esse código ta gerando um pdf desconfigurado

Gemini said
O principal motivo para o PDF estar "desconfigurado" (textos sobrepostos, quebras de página erradas ou formatação estranha) no uso do FPDF com Streamlit geralmente se deve a três fatores no seu código:

Cálculo de Altura em Tabelas (MultiCell): Quando você usa multi_cell (para textos longos que quebram linha) ao lado de cell (texto curto), o cursor do PDF desce, e o set_xy manual perde a referência se houver uma quebra de página automática.

Caracteres Especiais: O latin-1 não suporta aspas inteligentes (criadas pelo Word/WhatsApp), travessões longos ou emojis, o que causa erros de codificação ("lixo" no texto).

Quebras de Página Manuais x Automáticas: Misturar if pdf.get_y() > 230 com multi_cell causa buracos ou sobreposição no rodapé.

Aqui estão as correções pontuais para estabilizar seu PDF.

1. Melhore a função de limpeza de texto
Substitua sua função clean_pdf_text por esta versão mais robusta. Ela remove caracteres que quebram o FPDF (como aspas de celular):
...
2. Crie uma função para "Linhas Inteligentes"
Adicione esta função logo após a criação da classe OfficialPDF. Ela é essencial para desenhar tabelas onde uma coluna tem muito texto e a outra pouco, mantendo tudo alinhado e gerenciando a quebra de página automaticamente.

Python
def print_row_multicell(pdf, col_widths, data_row, align='L', border=1):
    """
    Imprime uma linha onde colunas podem ter alturas diferentes (MultiCell).
    col_widths: lista com largura das colunas [100, 30, 60]
    data_row: lista com os textos ['Pergunta longa...', 'Sim', 'Obs...']
    """
    max_h = 0
    font_size = pdf.font_size
    
    # 1. Calcular a altura necessária para a linha inteira
    for i, text in enumerate(data_row):
        # Quantas linhas esse texto vai ocupar?
        width = col_widths[i]
        # Truque para estimar altura: get_string_width não considera quebra de palavra,
        # então usamos uma heurística ou simulamos.
        # No FPDF simples, a melhor forma segura é calcular baseada na largura:
        num_lines = pdf.get_string_width(text) / width
        line_height = 5 # Altura base da linha
        h = max(line_height, (int(num_lines) + 1) * line_height)
        # Se tiver muitas quebras de linha manuais (\n)
        h += text.count('\n') * line_height
        if h > max_h:
            max_h = h

    # 2. Verificar se cabe na página, senão cria nova
    if pdf.get_y() + max_h > 270: # 270 é margem de segurança para A4
        pdf.add_page()
        
    # 3. Imprimir as células
    x_start = pdf.get_x()
    y_start = pdf.get_y()
    ...
    for i, text in enumerate(data_row):
        ...
        # Usa MultiCell para tudo para garantir alinhamento vertical
        pdf.multi_cell(w, 5, text, border=border, align=align)
        # Atualiza X para a próxima coluna
        x_start += w
        
    # 4. Move o cursor para baixo baseado na altura máxima calculada
    pdf.set_xy(10, y_start + max_h) # 10 é a margem esquerda padrão
3. Corrija a Seção de "Checklist" (A mais crítica)
No seu código, dentro da aba Estudo de Caso -> Gerar PDF, substitua o loop do Checklist por este código usando a função acima. Isso resolverá a desconfiguração visual.

Substitua o trecho do checklist_items no PDF por:

Python
            # ... (cabeçalho da tabela anterior) ...
            pdf.set_fill_color(220, 220, 220); pdf.set_font("Arial", "B", 9)
            # Definir larguras fixas
            w_perg = 110
            w_opt = 25
            w_obs = 55 # O resto da largura (190 total - 110 - 25)
            
            pdf.cell(w_perg, 8, "PERGUNTA / ASPECTO OBSERVADO", 1, 0, 'C', 1)
            pdf.cell(w_opt, 8, "SIM/NÃO", 1, 0, 'C', 1)
            pdf.cell(w_obs, 8, clean_pdf_text("OBSERVAÇÕES"), 1, 1, 'C', 1)
            ...
            for item in checklist_items:
                ...
                txt_col1 = clean_pdf_text(item)
                txt_col2 = clean_pdf_text(opt)
                txt_col3 = clean_pdf_text(obs)
                
                # CHAMA A FUNÇÃO DE LINHA INTELIGENTE
                # Note: Ajuste as larguras [110, 25, 55] conforme sua página
                print_row_multicell(pdf, [110, 25, 55], [txt_col1, txt_col2, txt_col3])
4. Correção Rápida no PEI (Flexibilização)
A tabela de flexibilização do PEI também costuma quebrar. Aplique a mesma lógica. Onde você tem pdf.cell lado a lado com pdf.multi_cell (conteúdos específicos), use a lógica de altura máxima.

No seu código atual, para a seção 7.1 FLEXIBILIZAÇÃO, você está usando apenas pdf.cell, o que corta o texto se ele for longo. Se quiser permitir quebra de linha nas colunas de Sim/Não ou no nome da disciplina, use a print_row_multicell lá também.

5. Dica de Performance do Streamlit
Notei que você está lendo a planilha inteira dentro da função login e load_db.
Para evitar que o sistema fique lento ou trave ao gerar o PDF, use o cache do Streamlit:

Python
@st.cache_data(ttl=60) # Cache por 60 segundos
def load_db_cached():
    return conn.read(worksheet="Alunos", ttl=0)

# Use load_db_cached() ao invés de conn.read direto nas funções de leitura
Resumo
O problema principal não é o Streamlit, mas a lógica linear do FPDF. O FPDF desenha como uma caneta no papel: se você escrever muito em uma coluna e pouco na outra, a "caneta" para em alturas diferentes. A função print_row_multicell sugerida acima força todas as colunas a terminarem na mesma altura (a altura da maior célula), garantindo que a próxima linha comece alinhada.

You said
esse fica fora da pagina à direita Quais objetos que gosta? Tem um objeto de apego?

Gemini said
O problema acontece porque o comando pdf.cell() não quebra linha automaticamente. Se o texto for um pouco mais longo do que a largura da página (ou se houver algum erro de cálculo de margem), ele ignora a margem direita e escreve para fora do papel.

Para corrigir isso, devemos trocar o pdf.cell por pdf.multi_cell também na pergunta/título, garantindo que o texto respeite a margem e quebre a linha se necessário.

Vá até a parte do código onde está a lista verbatims (dentro da aba PEI, seção 3. Protocolo de Conduta no PDF) e substitua o loop for por este:

Python
            # ... (código anterior da seção 3) ...
...
O que mudou?
pdf.multi_cell(0, 7, ...) na Pergunta: Antes você usava pdf.cell. O multi_cell força o texto "Quais objetos que gosta..." a quebrar para a linha de baixo caso ele atinja a margem direita, evitando que saia da página.

align='L': Garante que o texto fique alinhado à esquerda.

You said
fpdf.errors.FPDFException: This app has encountered an error. The original error message is redacted to prevent data leaks. Full error details have been recorded in the logs (if you're on Streamlit Cloud, click on 'Manage app' in the lower right of your app).

Traceback:

File "/mount/src/integraaee/app_pei.py", line 900, in <module>

pdf.multi_cell(0, 6, clean_pdf_text(v if v else "---"), "LRB")

~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

File "/home/adminuser/venv/lib/python3.13/site-packages/fpdf/fpdf.py", line 242, in wrapper
...
File "/home/adminuser/venv/lib/python3.13/site-packages/fpdf/fpdf.py", line 4550, in multi_cell

text_line = multi_line_break.get_line()

File "/home/adminuser/venv/lib/python3.13/site-packages/fpdf/line_break.py", line 792, in get_line
...
Gemini said
Esse erro (Not enough horizontal space to render a single character) acontece porque o FPDF "se perdeu" na posição horizontal (eixo X).

Quando você usa w=0 no multi_cell, ele diz: "Use todo o espaço disponível da posição atual até a margem direita". Se, por algum motivo, o cursor estiver encostado na direita (ou fora da margem), o espaço disponível é zero ou negativo, e o sistema trava.

A correção é simples: force o cursor a voltar para a esquerda antes de imprimir a resposta.

Substitua o loop for l, v in verbatims: inteiro por este código blindado:
...
Python
            ...
            for l, v in verbatims:
                ...
                # "LBR" faz borda Esquerda, Baixo, Direita (fechando a caixa)
                pdf.multi_cell(0, 6, clean_pdf_text(v if v else "---"), border="LBR", align='L', fill=False)
O que mudou para corrigir:
pdf.set_x(10): Adicionei essa linha antes da pergunta e antes da resposta. Isso garante que, não importa onde o cursor parou na linha anterior, ele sempre começará a caixa encostado na margem esquerda (assumindo que sua margem seja 10mm, que é o padrão do seu código).

Verificação de Página (if pdf.get_y() > 250): Adicionei uma checagem de segurança. Se o cursor estiver muito embaixo, ele cria uma página nova antes de tentar escrever a pergunta, evitando que a pergunta fique numa página e a resposta na outra (ou que trave tentando escrever no rodapé).
...
esse também ta com o mesmo problema

Gemini said
O erro é exatamento o mesmo: você usou w=0 (largura zero/automática) na primeira célula ("Médico Responsável").

No FPDF, w=0 significa "Ocupe todo o espaço até a margem direita".

O código desenha "Médico Responsável" ocupando a linha inteira.

O cursor para na margem direita.

Quando ele tenta desenhar o nome do médico, não existe mais papel (espaço horizontal = 0), e ele trava.

A Solução
Você precisa dar uma largura fixa para o rótulo ("Médico Responsável") e deixar a largura automática (0) apenas para o valor (o nome do médico).

Substitua aquela linha inteira por isto:

Python
# Largura fixa de 50mm para o rótulo (pode ajustar se quiser)
pdf.set_font("Arial", "B", 12)
# w=50, h=8, borda=1, ln=0 (Fica na mesma linha)
pdf.cell(50, 8, "Médico Responsável:", 1, 0, 'L') 

# Largura automática (0 = resto da linha) para o nome
pdf.set_font("Arial", "", 12)
# w=0, h=8, borda=1, ln=1 (Pula linha no final)
pdf.cell(0, 8, clean_pdf_text(data.get('med_doc', 'N/A')), 1, 1, 'L')
Resumo da mudança:

Troquei o primeiro pdf.cell(0, ...) por pdf.cell(50, ...) (para ele não comer a linha toda).

Adicionei ln=0 no primeiro cell para garantir que o cursor não pule a linha antes da hora.

Mantive w=0 apenas no segundo cell (o nome do médico), para ele preencher o resto da linha.
...
Gemini said
O problema persiste porque o cursor do PDF (a "caneta") provavelmente parou no final da linha anterior (ou fora da margem direita) por causa do comando anterior (multi_cell ou cell).

Quando você manda desenhar a célula do "Médico" (pdf.cell(50...)), ele começa de onde o cursor parou. Se o cursor parou na direita, ele desenha mais 50mm para fora da folha. Aí, quando chega na célula seguinte com w=0 ("resto da página"), o cálculo dá negativo e o sistema trava ou desconfigura.

A solução é forçar a "caneta" a voltar para a esquerda antes de começar essa linha.

Substitua a sua linha problemática por este bloco de 3 linhas:

Python
# 1. Garante que começa na margem esquerda e numa linha nova
pdf.ln(10)  # Pula uma linha para dar espaço do bloco anterior
pdf.set_x(10) # Força o cursor a voltar para a esquerda (assumindo margem 10mm)

# 2. Desenha o rótulo (Com ln=0 explícito para não pular linha ainda)
pdf.set_font("Arial", "B", 12)
pdf.cell(50, 8, clean_pdf_text("Médico Responsável:"), border=1, ln=0, align='L')

# 3. Desenha o valor (Com ln=1 para pular linha no final)
pdf.set_font("Arial", "", 12)
# Ajustei a altura para 8 (igual ao rótulo) para ficar alinhado
pdf.cell(0, 8, clean_pdf_text(data.get('med_doc', 'N/A')), border=1, ln=1, align='L')
O que estava errado no seu código:
Falta de Reset: Você não disse para o PDF "volte para a esquerda" (set_x(10)). Ele continuou escrevendo onde parou o texto anterior.

Alturas Diferentes: Você usou h=8 no rótulo e h=10 no nome. Isso cria um "degrau" feio na tabela. Mudei ambos para 8.

Argumentos Implícitos: É mais seguro usar ln=0 e ln=1 explicitamente para garantir que o FPDF não se confunda.

You said
Me de só a linha pra eu copiar e colar sobre a outra

Gemini said
Aqui está a linha exata, já com o comando para pular linha antes (ln) e com as larguras/alturas corrigidas para não travar:
...
Gemini said
O problema é o mesmo: o cell(0, ...) tenta usar a largura restante, mas se o cursor não estiver exatamente na margem esquerda, ele trava ou joga o texto para fora.

Copie e cole esta linha no lugar. Ela força o cursor a voltar para a esquerda (set_x(10)) antes de escrever:
...
Gemini said
Aqui estão as linhas corrigidas. O segredo é sempre forçar o cursor a voltar para a margem esquerda (set_x(10)) antes de desenhar caixas que ocupam a largura toda (w=0).

Copie e substitua o bloco inteiro por este:
...
pdf.ln(2)
O que eu fiz:
Adicionei pdf.set_x(10); no início das duas linhas de desenho. Isso garante que o PDF não tente desenhar fora da folha caso o cursor tenha se perdido na linha anterior.

You said
Agora eu quero fixar a barra lateral, pra não pertmiri fechar ela

Gemini said
Para impedir que a barra lateral seja fechada, precisamos usar um "truque" de CSS, pois o Streamlit não tem um comando nativo para travar a barra.

O que faremos é esconder o botão de "X" (que fecha a barra) e garantir que ela inicie aberta.

Procure no seu código onde você tem st.set_page_config e logo abaixo dele (ou dentro do seu bloco de style existente), adicione este código CSS:

Python
# --- CONFIGURAÇÃO DA PÁGINA (Certifique-se que está como 'expanded') ---
# Se já tiver essa linha, apenas confira o initial_sidebar_state
...
Recomendo colocar logo após o seu st.set_page_config, antes de começar a desenhar a interface.

Nota importante: Ao fazer isso, em telas muito pequenas (celulares), a barra lateral pode cobrir o conteúdo e o usuário não conseguirá fechá-la. Para uso em desktop/notebook (que parece ser o caso escolar), funciona perfeitamente.
...
Deveria vir depois do nome do sistema

Gemini said
Para mover o nome do docente para baixo do título do sistema, você precisa fazer duas alterações simples:

1. Remover a linha antiga
Logo após a função login(), apague ou comente esta linha que está solta no código:

Python
# --- APAGUE ESTA LINHA ---
# st.sidebar.markdown(f"👤 **Docente:** {st.session_state.get('usuario_nome', 'Professor')}")
2. Adicionar no lugar certo (Dentro da Barra Lateral)
Agora, vá para a seção # --- BARRA LATERAL --- e substitua o início dela por este código. Eu adicionei o nome do docente logo abaixo do subtítulo, centralizado:
...
with st.sidebar:
    ...
    # --- AQUI ESTÁ A NOVA POSIÇÃO DO NOME ---
    nome_prof = st.session_state.get('usuario_nome', 'Professor')
    st.markdown(f"""
        <div style="text-align: center; margin-top: 8px; font-size: 0.9rem; color: #475569; background-color: #f1f5f9; padding: 5px; border-radius: 5px;">
            👤 <b>Docente:</b> {nome_prof}
        </div>
    """, unsafe_allow_html=True)
    # ----------------------------------------
    ...
    # ... (o resto do código continua igual: default_doc_idx = 0 ...)
O que mudou:
Eu coloquei o nome dentro de uma div centralizada com um fundo cinza claro (#f1f5f9), para ficar visualmente integrado ao cabeçalho da barra lateral, logo abaixo de "Gestão de Educação Especial".

You said
quero reformar esse menu

Gemini said
Com certeza, Victor. Vamos dar uma "geral" nessa barra lateral para deixá-la com cara de aplicativo profissional (SaaS).

Os principais problemas do layout atual são:

Informação espalhada: O nome do docente está "flutuando".

Hierarquia visual: Tudo tem o mesmo peso.

Botões de perigo: O botão "Excluir" está muito exposto, perigoso de clicar sem querer.

Aqui está uma proposta de Reforma Completa da Sidebar, organizando em: Cabeçalho (Identidade) -> Painel de Controle (Seletores) -> Rodapé (Ações).

Copie e substitua todo o bloco with st.sidebar: por este código:
...
Card do Docente: Em vez de um texto solto, agora é um "cartão" cinza claro com bordas arredondadas. Fica muito mais profissional e deixa claro quem está logado.

Títulos Claros: Usei st.markdown("### 🎓 Estudante") em vez de labels padrão do Streamlit, o que cria uma hierarquia de leitura melhor.

Label Visibility="Collapsed": Removi os labels pequenos repetitivos ("Selecione o Estudante") e deixei o título da seção falar por si.

Botões Lado a Lado: Coloquei "Sair" e "Excluir" na mesma linha no final. O "Sair" ganha destaque (primary) e o "Excluir" fica mais discreto (secondary) para evitar acidentes.

You said
aqui a tela está em 100% queria que tudo aparesse aqui sem precisar rolar a tela pra baixo do menu

Gemini said
Para resolver o problema de "rolagem" e fazer tudo caber na tela (100%), precisamos compactar o layout. O Streamlit coloca muitos espaços em branco por padrão, e o "Card" que criamos antes estava bonito, mas muito alto.

Vamos aplicar um "Layout Compacto":

Reduzir as Margens (CSS): Forçar os elementos a ficarem mais próximos.

Afinar o Card do Usuário: Transformá-lo em uma linha fina.

Remover Espaços Vazios: Tirar os <br> e divisores desnecessários.

Textos Mais Curtos: Simplificar os rótulos.

Substitua todo o bloco with st.sidebar: por este código otimizado:
    ...
    with c2:
        if selected_student != "-- Novo Registro --":
            if st.button("🗑️ Excluir", type="secondary", use_container_width=True):
                st.session_state.confirm_delete = True
O que mudou:
CSS gap: 0.5rem: Isso é o mais importante. O padrão do Streamlit é espaçar muito os widgets. Esse comando força eles a ficarem coladinhos.

Nome "Slim": Tirei aquele cartão grande com título "Docente Logado" e transformei numa barra cinza fina apenas com o ícone e o nome abreviado (Primeiro + Último).

Títulos Menores: Usei <p class="section-label"> em vez de ### Título (H3), pois o H3 ocupa muito espaço vertical.

Sem Espaçadores: Removi todos os <br> e espaços extras. Agora o menu deve terminar logo após os botões, cabendo perfeitamente na tela.

Gemini said
Para fazer tudo caber na tela sem barra de rolagem (o famoso "viewport height" de 100%), precisamos ser agressivos no CSS para remover o acolchoamento (padding) padrão que o Streamlit coloca no topo e entre os elementos.

Aqui está o código da barra lateral Ultra-Compacta.

Substitua todo o bloco with st.sidebar: por este:
...
with st.sidebar:
    ...
    with c2:
        if selected_student != "-- Novo Registro --":
            if st.button("🗑️ Excluir", type="secondary", use_container_width=True):
                st.session_state.confirm_delete = True
O que mudou para caber na tela (100%):
padding-top: 1rem: O Streamlit, por padrão, coloca um espaço enorme no topo da barra. Eu forcei esse espaço a diminuir drasticamente.

gap: 0.3rem: Diminui o espaço entre o título, o selectbox, o radio button, etc. Eles ficam bem próximos.

Fontes Menores: Reduzi levemente o tamanho das fontes dos títulos (.section-label) e do subtítulo para ganhar pixels verticais.

Remoção de Margens: Forcei margin: 0 nos títulos para que eles não empurrem o conteúdo para baixo.

Agora o menu deve ficar fixo e visível inteiro, mesmo em telas de notebooks menores (1366x768).

You said
Cada coisa que preencho ele salva, isso as vezes buga pq eu to digitando a tela fica cinza e as vezxes perco o que to digitando

Gemini said
Isso acontece porque o Streamlit é "reativo": por padrão, toda vez que você clica fora de um campo, ele recarrega o código inteiro para salvar o valor. Além disso, aquele código de Autosave (st_autorefresh) que colocamos antes pode estar recarregando a página enquanto você ainda está digitando.

Para resolver isso de vez e deixar o preenchimento fluido (igual a um formulário normal que só envia no final), precisamos fazer duas coisas:

Desativar o Autosave (ele interrompe a digitação).

Usar Blocos de Formulário (st.form): Isso segura os dados e só envia quando você clica em um botão "Confirmar/Salvar" dentro da aba.

Aqui está como corrigir.

Passo 1: Desative o Salvamento Automático
Vá lá no topo do seu código (perto da linha 190) e apague ou comente estas linhas. Elas são as culpadas pela tela ficar cinza "do nada" enquanto você digita:

Python
# --- APAGUE OU COMENTE ISSO ---
# from streamlit_autorefresh import st_autorefresh
# count = st_autorefresh(interval=60000, key="autosave_counter")
# if count > 0:
#    auto_save()
Passo 2: Aplique "Formulários" nas Abas
Vou reescrever a parte do Estudo de Caso (que é onde paramos) colocando cada aba dentro de um formulário. Assim, você preenche tudo e só recarrega quando clicar no botão no final da aba.

Substitua a parte das abas do ESTUDO DE CASO por este código blindado:
