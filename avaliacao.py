...
GESTAO_MATRICULAS = {
    "8257601", "8844051", "8084912", "8829405", "8011512", "8258411", "7047682", "88286861"
}
...
def _ano_da_turma(turma: str) -> Optional[int]:
    m = re.search(r"([1-5])\s*[ºO]?\s*ANO", _norm(turma))
...
# -----------------------------------------------------------------------------
# Repositório Supabase: uma tabela JSON, compatível com o padrão já usado no app
# -----------------------------------------------------------------------------
@dataclass
class Contexto:
    ano_letivo: int
...
class AvaliacaoRepo:
    ...
    def listar_turma_todos_componentes(
        self, ano_letivo: int, ano_escolar: int, trimestre: int, turma: str
    ) -> List[Dict[str, Any]]:
        try:
            res = (
                self.supabase.table(TABLE_NAME)
                .select("componente,aluno_nome,dados_json")
                .eq("tipo", "aluno")
                .eq("ano_letivo", ano_letivo)
                .eq("ano_escolar", ano_escolar)
...
def _eh_gestor(matricula: str, nome: str = "", supabase: Any = None) -> bool:
    ...
    if str(st.session_state.get("user_role", "")).strip().lower() in {"admin", "gestor", "diretor", "coordenador"}:
        return True
    # Fallback para instalações antigas sem matriz_gestao configurada.
    return nome_norm in {
        _norm("José Victor Souza Gallo"),
        _norm("Luciana Lopes Faber"),
        _norm("Oelen Fernando Pedro"),
        _norm("Luciana Martinati Tetzner"),
        _norm("Noreh Cristina Heldt Aldrigui"),
        _norm("Marília Motta Camargo dos Reis"),
    }
...
def _turmas_permitidas(
    ...
    if gestor or not turmas:
        try:
            ...
            pass
    # preserva grafia mais bonita da matriz quando houver duplicatas por caixa/acentos
    seen = set()
    out = []
    for t in turmas:
        k = _norm(t)
...
def _componentes_permitidos(
    matriz_professores: Optional[pd.DataFrame], usuario_nome: str,
    gestor: bool, turma: str, disponiveis: List[str]
) -> List[str]:
...
def calcular_conceito_sugerido(
    ...
    if not at:
        return {
            "conceito": "",
            "motivo": "Ainda não há dimensões marcadas como trabalhadas e avaliadas (AT).",
            "alertas": ["Defina a cobertura curricular antes de fechar o conceito."],
            "metricas": {},
        }
    ...
    if len(julgaveis) < min_evid:
        return {
            "conceito": "",
            "motivo": f"Há evidência julgável em {len(julgaveis)} de {len(at)} dimensões AT.",
            "alertas": ["Produza novas evidências antes da conceituação final."],
    ...
    if any(s == "NC" for s in estr_status.values()) or pct_nc >= 0.25 or (nc > 0 and pct_cons < 0.50):
        conceito = "AB"
    ...
    elif aa / total >= 0.50 and conceito == "AD" and not avancado_ok:
...
def gerar_pdf_perfil(
    aluno: str, turma: str, ano_letivo: int, trimestre: int,
    componentes: Sequence[str], registros: Mapping[str, Mapping[str, Any]],
...
def gerar_pdf_turma(
    turma: str, ano_letivo: int, trimestre: int,
    componentes: Sequence[str], resumo: Mapping[str, Mapping[str, Any]],
) -> bytes:
    pdf = _PDF()
    pdf.set_auto_page_break(True, 15)
    pdf.add_page()
    ...
    for comp in componentes:
        ...
        pdf.multi_cell(0, 5, _clean_pdf(
            f"AB: {cont.get('AB',0)} | B: {cont.get('B',0)} | AD: {cont.get('AD',0)} | A: {cont.get('A',0)} | Sem fechamento: {cont.get('-',0)}"
        ))
...
def _default_cobertura(base: Mapping[str, Any]) -> Dict[str, Any]:
    return {
        d["codigo"]: {"situacao": "AT", "recomposicao": "", "observacao": ""}
...
def _curriculo_tab(repo: AvaliacaoRepo, ctx: Contexto, base: Mapping[str, Any]):
    ...
    for d in base.get("dimensoes", []):
        ...
        st.markdown(
            f'<div class="av-card"><span class="av-code">{cod}</span>{badge}<b>{d["nome"]}</b><br><span class="av-muted">{d["demonstrar"]}</span></div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns([1, 2])
        with c1:
            situacao = st.selectbox(
                f"Situação curricular · {cod}",
                SITUACAO_OPCOES,
                index=SITUACAO_OPCOES.index(atual.get("situacao", "AT")) if atual.get("situacao", "AT") in SITUACAO_OPCOES else 0,
                format_func=lambda x: f"{x} — {SITUACAO_CURRICULAR[x]}",
                key=f"av_cov_{ctx.turma}_{ctx.componente}_{cod}",
            )
        p = pres.get(cod, {})
        with c2:
            recomposicao = st.text_input(
                f"Recomposição selecionada · {cod}",
                value=atual.get("recomposicao", ""),
                placeholder="Ex.: retomar valor posicional do 4º ano",
                key=f"av_rec_{ctx.turma}_{ctx.componente}_{cod}",
            )
        ...
        obs = st.text_area(
            f"Observação pedagógica · {cod}", value=atual.get("observacao", ""),
            height=60, key=f"av_covobs_{ctx.turma}_{ctx.componente}_{cod}",
        )
    ...
    with c1:
        percurso = st.selectbox(
            "Percurso curricular predominante da turma",
            PERCURSO_OPCOES,
            index=PERCURSO_OPCOES.index(percurso_salvo) if percurso_salvo in PERCURSO_OPCOES else PERCURSO_OPCOES.index(percurso_sugerido),
            format_func=lambda x: "— selecione —" if not x else f"{x} — {PERCURSOS[x]}",
            key=f"av_percurso_turma_{ctx.turma}_{ctx.componente}",
        )
    with c2:
...
def _blueprint_tab(repo: AvaliacaoRepo, ctx: Contexto, base: Mapping[str, Any]):
    ...
    for item in base.get("blueprint", []):
        ...
        rows.append({
            ...
            "Situação/evidência mínima recomendada": item["situacao"],
            "Bloom": item["bloom"],
            "O que deve revelar": item["evidencia"],
            "Incluída": bool(e.get("incluida", False)),
            "Instrumento/questão": e.get("instrumento", ""),
        })
    ...
    edited = st.data_editor(
        ...
        disabled=["Sit.", "Dimensão", "Situação/evidência mínima recomendada", "Bloom", "O que deve revelar"],
        column_config={
            "Incluída": st.column_config.CheckboxColumn("Incluída?"),
            "Instrumento/questão": st.column_config.TextColumn("Onde aparece?", width="medium"),
            "Situação/evidência mínima recomendada": st.column_config.TextColumn(width="large"),
            "O que deve revelar": st.column_config.TextColumn(width="large"),
        },
        key=f"av_blue_editor_{ctx.turma}_{ctx.componente}",
    )
    ...
    if st.button("💾 Salvar mapa da avaliação", type="primary", key=f"save_blue_{ctx.turma}_{ctx.componente}"):
        ...
        for _, r in edited.iterrows():
            itens[str(r["Sit."])] = {
                "incluida": bool(r["Incluída"]),
                "instrumento": str(r.get("Instrumento/questão", "") or ""),
            }
...
def _tabulacao_tab(repo: AvaliacaoRepo, ctx: Contexto, base: Mapping[str, Any], alunos: List[str]):
    ...
    edited = st.data_editor(
        ...
        hide_index=True,
        use_container_width=True,
        disabled=["Estudante", "Sugestão"],
        column_config=cfg,
        key=f"av_tab_{ctx.turma}_{ctx.componente}",
    )
    ...
    if st.button("💾 Salvar tabulação e recalcular sugestões", type="primary", key=f"save_tab_{ctx.turma}_{ctx.componente}"):
        ...
        for _, row in edited.iterrows():
            ...
            novo.update({
                "status": statuses,
                "conceito_sugerido": sug.get("conceito", ""),
                "motor": sug,
                "percurso": perc,
            })
...
def _conceituacao_tab(repo: AvaliacaoRepo, ctx: Contexto, base: Mapping[str, Any], alunos: List[str]):
    ...
    for nome in alunos:
        ...
        rows.append({
            "Estudante": nome,
            "Sugestão": motor.get("conceito", ""),
            "Percurso": perc,
            "Conceito final": d.get("conceito_final", "") or motor.get("conceito", ""),
            "Próximo passo": d.get("proximo_passo", ""),
            "Justificativa (se divergir)": d.get("justificativa", ""),
        })
    df = pd.DataFrame(rows)
    edited = st.data_editor(
        df,
        ...
        disabled=["Estudante", "Sugestão"],
        column_config={
            "Estudante": st.column_config.TextColumn(width="large"),
            "Sugestão": st.column_config.TextColumn(width="small"),
            "Percurso": st.column_config.SelectboxColumn(options=PERCURSO_OPCOES),
            "Conceito final": st.column_config.SelectboxColumn(options=[""] + CONCEITOS, width="small"),
            "Próximo passo": st.column_config.TextColumn(width="large"),
            "Justificativa (se divergir)": st.column_config.TextColumn(width="large"),
        },
        key=f"av_conc_{ctx.turma}_{ctx.componente}",
    )
    ...
    if st.button("✅ Confirmar conceitos e percursos", type="primary", disabled=bool(diffs), key=f"save_conc_{ctx.turma}_{ctx.componente}"):
        ...
        for _, r in edited.iterrows():
            ...
            d.update({
                ...
                "conceito_final": ("" if pd.isna(r["Conceito final"]) else str(r["Conceito final"] or "")),
                "percurso": perc,
                "proximo_passo": ("" if pd.isna(r["Próximo passo"]) else str(r["Próximo passo"] or "")),
                "justificativa": ("" if pd.isna(r["Justificativa (se divergir)"]) else str(r["Justificativa (se divergir)"] or "")),
                "motor": motor,
            })
...
def _painel_tab(repo: AvaliacaoRepo, ctx_base: Contexto, componentes: List[str], alunos: List[str]):
    ...
    for comp, r in resumos.items():
        c = r["conceitos"]
        linhas.append({
            "Componente": comp, "AB": c["AB"], "B": c["B"], "AD": c["AD"], "A": c["A"], "Sem fechamento": c["-"],
            "Prioridades coletivas": "; ".join(r["prioridades"][:3]),
        })
...
def _perfil_tab(repo: AvaliacaoRepo, ctx_base: Contexto, componentes: List[str], alunos: List[str]):
    ...
    for comp in componentes:
        ...
        linhas.append({
            "Componente": comp,
            "Conceito": d.get("conceito_final") or d.get("conceito_sugerido") or "—",
            "Percurso": d.get("percurso", "—"),
            "O que já sabe": "; ".join(fortes[:4]),
            "O que precisa consolidar": "; ".join(prioridades[:4]),
            "Próximo passo": d.get("proximo_passo", ""),
        })
    st.dataframe(pd.DataFrame(linhas), hide_index=True, use_container_width=True)
...
def _conselho_tab(repo: AvaliacaoRepo, ctx_base: Contexto, componentes: List[str], alunos: List[str]):
    ...
    por_aluno = []
    comp_col = {
        "Língua Portuguesa": "LP", "Matemática": "M", "História": "H", "Geografia": "G", "Ciências": "C"
    }
    ...
    if st.button("➡️ Enviar síntese para Nova Ata de Conselho", type="primary", key=f"av_to_ata_{ctx_base.turma}"):
        ...
        for _, r in abaixo.iterrows():
            rows_ab.append({
                "Estudante": r.get("Estudante", ""),
                "LP": r.get("LP", ""), "M": r.get("M", ""), "H": r.get("H", ""), "G": r.get("G", ""), "C": r.get("C", ""),
                "A": "", "EF": "", "LT": "", "LIBRAS": "",
            })
        rows_b = []
        for _, r in basico.iterrows():
            if r.get("LP") == "B" or r.get("M") == "B":
                acoes = []
                if r.get("LP") == "B": acoes.append("LP")
                if r.get("M") == "B": acoes.append("Matemática")
